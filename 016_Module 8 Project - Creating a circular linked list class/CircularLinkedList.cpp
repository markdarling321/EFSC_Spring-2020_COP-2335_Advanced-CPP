...
{
	cout << " CircularLinkedList() DEFAULT CONSTRUCTOR\n";
	head = nullptr;
}
...
{
	if (!head)									// IF HEAD DOES NOT EXIST
		return true;							// LIST DOES NOT EXIST

	else										// OTHERWISE LIST MUST EXIST
		return false;							// AND CONTAIN AT LEAST 1 NODE
}
///////////////////////////////////////////////////////////////////////////////
void CircularLinkedList::appendNode(int num)
{
	cout << " appendNode(int)\n";
	ListNode* newNode;				// DEFINE SELF-REFERENTIAL NEW NODE OBJECT
	ListNode* nodePtr;				// CURRENT NODE POINTER FOR TRAVERSING LIST

	// CREATE NEW NODE AND UPDATE VALUE WITH DATA
	newNode = new ListNode;				// CREATE NEW NODE
	newNode->value = num;				// UPDATE NEW NODE'S VALUE WITH DATA
	newNode->next = nullptr;			// INITIALIZE NEW NODE'S LINK

	// NEXT WE DETERMINE WHERE IN OUR LIST TO APPEND OUR NEWLY-CREATED NODE

	if (!head)				// IF LIST IS EMPTY AND DOES NOT CONTAIN ANY NODES
	{						// MAKE newNode THE FIRST NODE

		head = newNode;					// UPDATE head TO POINT TO NEW NODE
		newNode->next = head;			// POINT CURRENT NODE POINTER AT HEAD
		cout << " NODE APPENDED TO HEAD OF LIST\n";
	}
	else	// IF LIST IS NOT EMPTY, INSERT newNode AT END OF LIST
	{
		nodePtr = head;					// INITIALIZE nodePtr TO HEAD OF LIST

		while (nodePtr->next != head)	// FIND LAST NODE IN LIST
		{
			nodePtr = nodePtr->next;	// TRAVERSE LIST
		}
		nodePtr->next = newNode;		// INSERT newNode AT END OF LIST
		newNode->next = head;
		cout << " NODE APPENDED TO LIST\n";
	}
}
///////////////////////////////////////////////////////////////////////////////
void CircularLinkedList::insertNode(int num)
{
	cout << " insertNode(int)\n";
...
///////////////////////////////////////////////////////////////////////////////
void CircularLinkedList::deleteNode(int num)
{
	cout << " deleteNode(int)\n";
	ListNode* nodePtr;				// CURRENT NODE POINTER FOR LIST TRAVERSAL
