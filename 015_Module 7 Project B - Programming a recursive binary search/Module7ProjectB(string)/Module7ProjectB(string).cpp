...
//			be used to search a string vector object. Also, write a program to
//			test your algorithm.
//
///////////////////////////////////////////////////////////////////////////////
...
void printLists(const vector<string>& list1, const vector<string>& list2,
	const vector<string>& list3, const vector<int>& list4);
void printStats(const vector<string>& list1, const vector<string>& list2,
	const vector<string>& list3, const vector<int>& list4,
...
int main()
{
	...
	vector<string> list1;	// BUBBLE SORT
	vector<string> list2;	// SELECTION SORT
	vector<string> list3;	// INSERTION SORT
	...
	// SORT LIST
	bubbleSort(list4, bubbleCompCount, bubbleAssCount);
	selectionSort(list4, selectCompCount, selectAssCount);
	insertionSort(list4, insertCompCount, insertAssCount);

	// COPY SORTED LIST TO OTHER STRING VECTOR LISTS
	for (int i = 0; i < listLength; i++)
	{
		temp = list4[i];
		list1.push_back(temp);
...
// MODIFIED binarySearch() THAT REPORTS ALL RESULTS
bool binarySearch2(const vector<int>& list, int searchItem, int& binary2CompCount,
	vector<int>& rList1)
{
	int first = 0;
											...
											// IS MORE THAN 1 RESULT
		{
			...
			while (found == true && temp >= 0)		// LOOK LEFT
			{
				...
				if (list[temp] == searchItem)			// CHECK TO SEE IF IT MATCHES searchItem
				{
					...
					cout << " [INDEX] " << temp << " PUSHED TO rList!\n\n";
				}
			...
			found = true;
			temp = mid;
			while (found == true && temp <= list.size())		// LOOK RIGHT
			{
				cout << " found == true && temp < list.size()\n";
